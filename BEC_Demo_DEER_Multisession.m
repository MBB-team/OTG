% This demonstration script is part of the OTG toolbox, used for generating and presenting a battery of economic choices.
    % Demonstration of the 4 types of costs. The participant takes part in two experimental sessions (e.g., before and after treatment, or before and
    % after some experimental manipulation). In the first session, the participant sees instructions, then examples, and then performs a choice
    % calibration. This is done for each of 4 cost types. 
    % In the follow-up session, the participant reviews a short instruction and performs a follow-up battery of choices.
