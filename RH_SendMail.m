%% Send e-mails to notify about Matlab progress
function RH_SendMail(title,text,appendix)

% Note: must allow access to less secure apps
%     myaccount.google.com/u/4/lesssecureapps

% Default e-mail info
email_address = 'prisme.treasureisland@gmail.com';
email_password = 'Pessiglione';

% Send e-mail after setting preferences and properties
...
sendmail2('roelandheerema@hotmail.com',title,text,appendix); 

end

%% Sendmail2
% A copy of the matlab function SENDMAIL, with the addition of the property setting in line 95
% which was missing in the original SENDMAIL and therefore failed. (RLH - April 2019)

function sendmail2(to,subject,theMessage,attachments)
...
%   addresses.  SUBJECT is a character vector.  MESSAGE is either a character vector or a cell
%   array of character vectors.  If it is a character vector, the text will
%   automatically wrap at 75 characters.  If it is a cell array, it won't wrap, but
%   each cell starts a new line.  In either case, use char(10) to explicitly specify
%   a new line. ATTACHMENTS is a character vector or a cell array of character
%   vectors listing files to send along with the message.  Only TO and SUBJECT are
%   required.
%
%   SENDMAIL relies on two preferences, "Internet:SMTP_Server", your mail server, and
%   "Internet:E_mail", your e-mail address.  Use SETPREF to set these before using
%   SENDMAIL.  The easiest ways to identify your outgoing mail server is to look at
%   the preferences of another e-mail application or consult your administrator.  If
%   you cannot find out your server's name, setting it to just 'mail' might work.  If
%   you do not set these preferences SENDMAIL will try to determine them
%   automatically by reading environment variables and the Windows registry.
%
%   Example:
...
if (nargin < 4) 
    attachments = [];
elseif (ischar(attachments)) && attachments ~= ""
...
% Since strjoin (used below) can only join either srting arrays or a cell arrays of
% character vectors, we have to convert the string to a character array
% if (isStringScalar(theMessage))
%    theMessage = char(theMessage);
% end
...
elseif (length(dir(file)) == 1)
    % Absolute path.
    fullPathToFile = file;
elseif ~isempty(matlab.internal.language.introspective.safeWhich(file))
    % A file on the path.
    fullPathToFile = matlab.internal.language.introspective.safeWhich(file);
elseif ~isempty(matlab.internal.language.introspective.safeWhich([file '.']))
